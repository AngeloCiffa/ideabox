angular.module("ideaBox")
	.controller("ideaDetailsCtrl", function($scope, $stateParams, $filter, ideaService){
		...
		{
			$scope.$watch(function() {return ideaService.status.lastVotedId}, function (lastVotedId){
				if($scope.data.selectedIdea != undefined					
					 && lastVotedId === $scope.data.selectedIdea._id)
				{
					refreshIdeaFunction();
